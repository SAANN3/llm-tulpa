...
Every provider shares `allowed_chat_ids` and `think` (whether a reply thinks before answering) on top of its own settings. Splitting a long reply into several platform messages ([`plugins/messaging/chunking.rs`](./src/plugins/messaging/chunking.rs)) is shared too — it only tracks blank lines and fenced code blocks, nothing platform-specific, so every provider reuses it and only implements its own last-resort hard-cut on top.
