...
| `ollama.context_length` | `32768` | Must match Ollama's own context window (`OLLAMA_CONTEXT_LENGTH` in the root `.env` — see `llm/README.md`) — drives the `num_predict` cap and the history-compaction thresholds. The two live in different files, so change them together. |
