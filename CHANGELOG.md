...
### Changed
- The database schema was rebuilt and normalized to 3NF for the per-user model: a `users`/`user_settings` split replaces the old global settings singleton, `chats` and `files` are owned by a user, message images/file-ids and the old plugin columns are normalized into join/side tables, and a single centralized migration (versioned by a `schema_meta` sentinel) drops and recreates everything on a version bump. **Upgrading from 0.1.4 wipes existing chats/settings — there is no data migration** (a deliberate, stated decision). Fresh installs are unaffected.
