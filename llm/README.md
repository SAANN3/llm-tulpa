...
`OLLAMA_CONTEXT_LENGTH` sets how much context the model gets. Set it here **and** as the same-named env var on the backend (see [`../backend/README.md`](../backend/README.md)) — the backend derives its own token budgeting (the per-request `num_predict` cap, history-compaction thresholds) from that value, so the two need to agree.
