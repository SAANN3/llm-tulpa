...
It's off unless you opt in: set `COMPOSE_PROFILES=mtp` in the root `.env` (see `.env.example`) and start via `./start-docker.sh`, which keeps `ollama` and `llama-mtp`/`mtp-proxy` from both loading a model onto the same GPU at once. Either way the backend still only ever talks to one address — point `ollama.url` in `backend/data/settings.json` at `http://localhost:11434` for Ollama or `http://localhost:11435` for `mtp-proxy` (`mtp-proxy` translates Ollama's wire format to `llama-mtp`'s OpenAI-compatible one, so the backend needs no code change either way). `MODEL_FILE`, `MMPROJ_FILE`, `OLLAMA_CONTEXT_LENGTH` and `OLLAMA_KV_CACHE_TYPE` all apply the same way to `llama-mtp` as to Ollama; `LLAMA_MTP_SPEC_DRAFT_N_MAX` controls how many tokens the draft head speculates ahead.
