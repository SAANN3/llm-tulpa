...
/**
 * Stashes a prompt (and its `think` toggle, as set on the home page's composer at send
 * time) so the chat page can auto-send it the moment it lands, right after creating a
 * new chat from the home page's composer — sessionStorage rather than a query param
 * since prompt text can be arbitrarily long (query params/URLs have practical length
 * limits; sessionStorage's per-origin quota is megabytes).
 */
export function setPendingPrompt(chatId: number, prompt: string, think: boolean): void {
  const value: PendingPrompt = { chatId, prompt, think, expiresAt: Date.now() + VALID_WINDOW_MS }
  sessionStorage.setItem(STORAGE_KEY, JSON.stringify(value))
}

function readPendingPrompt(chatId: number): { prompt: string; think: boolean } | null {
  ...
  return { prompt: parsed.prompt, think: parsed.think }
}
 ...
 */
export function peekPendingPrompt(chatId: number): { prompt: string; think: boolean } | null {
 ...
 */
export function consumePendingPrompt(chatId: number): { prompt: string; think: boolean } | null {
