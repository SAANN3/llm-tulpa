...
/** Trims and drops empty entries from an array field's values */
const cleanList = (items: string[]): string[] => items.map((item) => item.trim()).filter((item) => item.length > 0);

const defaultTextFor = (type: PropertyType): string => type === 'object' ? '{}' : '';

/** A short description of what kind of value a field expects, with an example */
const typeHint = (type: PropertyType): string | null => {
    switch (type) {
        case 'string':
...
/** A settings form built generically from a plugin's own schema */
export const PluginSettings = ({plugin, getSchema, getHelp, onSave, onBack}: PluginSettingsProps) => {
    ...
    return (
        <Div className="vbox plugin-settings">
            ...
            <Div className="vbox plugin-settings__scroll">
                {help ? (
                    <Div className="vbox plugin-settings__help">
                        ...
                        {helpExpanded ?
                            <Label variant="secondary" className="plugin-settings__help-text" text={help}/> : null}
                    </Div>
                ) : null}
                    ...
                    schema.map((field) => (
                        <Div className="vbox plugin-settings__field" key={field.name}>
                            <Div className="plugin-settings__field-head">
                                ...
                                {field.property_type === 'boolean' ? (
                                    <ToggleSwitch
                                        ...
                                        onToggled={(toggled) => setBoolValues((prev) => ({
                                            ...prev,
                                            [field.name]: toggled
                                        }))}
                                    />
                                ...
                                <Div className="vbox plugin-settings__array">
                                    {(arrayValues[field.name] ?? []).map((item, index) => (
                                        ...
                                        <Div key={index} className="plugin-settings__array-row">
                                            ...
                                            />
                                            <Div className="plugin-settings__row-button"
                                                 onClick={() => removeArrayItem(field.name, index)}>
                                                <Minus width={16} height={16}/>
                                            </Div>
                                        </Div>
                                    ))}
                                    <Div className="plugin-settings__row-button plugin-settings__row-button--full"
                            ...
                            {typeHint(field.property_type) ? (
                                <Label className="mono plugin-settings__hint" variant="secondary"
                                       text={typeHint(field.property_type)!}/>
                            ) : null}
            ...
            <Div className="plugin-settings__actions">
                ...
                <Button className="plugin-settings__action" text="Save" onClicked={onSubmit}
                        disabled={schema === null || saving}/>
            </Div>
        </Div>
    )
};
